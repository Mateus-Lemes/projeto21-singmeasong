...
describe("Up Vote (/recommendation/:id/upvote)", ()=> {
    it("upvote added successfully", async () => {
        ...
        const response = await supertest(app).post(`/recommendations/${recomendationExist.id}/upvote`);
    ...
    it("vote entered in the bank", async ()=> {
        ...
        await supertest(app).post(`/recommendations/${createdRecommendation.id}/upvote`);
